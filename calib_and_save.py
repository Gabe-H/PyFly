...
# Final configuration to store the calibration data and auto-home at startup
for drive in drives:
    for axis in (drive.axis0, drive.axis1):
        # Tell the ODrive that the motor and encoder are already calibrated
        axis.motor.config.pre_calibrated = True
        axis.encoder.config.pre_calibrated = True

        # Search for index, home, then enter CLOSED_LOOP_CONTROL at startup
        axis.config.startup_encoder_index_search = True
        axis.config.startup_homing = True
        axis.config.startup_closed_loop_control = True
    # Idle all motors so that the configuration can be saved
    requestAxisStates(drive, AxisState.IDLE)
        
input('Done applying configuration, please run "odrvX.save_configuration()" in ODriveTool for each ODrive. Press Enter to continue.')
